...
int countRedun(struct digit * start) {
    int redundancies = 0;
    int digitCount[10] = {0};  // Array to keep track of the occurrence of each digit (0-9)

    struct digit *ptr = start;

    while (ptr != NULL) {
        int currentDigit = ptr->num;

        // Check if the current digit has occurred before
        if (digitCount[currentDigit] > 0) {
            redundancies++;
        }
